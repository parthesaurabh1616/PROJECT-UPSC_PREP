...
export const Title: React.FC<PrimProps> = ({ props, theme: t }) => {
  ...
  return (
    <Wrap t={t}>
      ...
      {props.subtitle && (
        <div style={{ ...b, fontFamily: FONT.body, fontSize: 40, color: t.accent }}>{props.subtitle}</div>
      )}
    </Wrap>
  );
};
  ...
  return (
    <div style={{ ...st, display: "flex", flexDirection: vertical ? "row" : "column", alignItems: vertical ? "center" : "flex-start", gap: 14, flex: vertical ? undefined : 1 }}>
      <div style={{ border: `1px solid ${t.line}`, background: t.bgSoft, borderRadius: 14, padding: "20px 24px", minWidth: vertical ? 620 : undefined }}>
        <div style={{ fontFamily: FONT.display, fontSize: 34, fontWeight: 600, color: t.ink }}>{step.label}</div>
        {step.note && <div style={{ fontFamily: FONT.body, fontSize: 22, color: t.ink3, marginTop: 6 }}>{step.note}</div>}
      </div>
        ...
        {cols.map((c, i) => {
          ...
          return (
            <div key={i} style={{ ...st, flex: 1, border: `1px solid ${t.line}`, background: t.bgSoft, borderRadius: 16, padding: 28 }}>
              <div style={{ fontFamily: FONT.display, fontSize: 40, fontWeight: 700, color: t.accent, marginBottom: 16 }}>{c.label}</div>
              {(c.points ?? []).map((p, j) => (
                <div key={j} style={{ fontFamily: FONT.body, fontSize: 26, color: t.ink2, marginBottom: 10, lineHeight: 1.35 }}>· {p}</div>
              ))}
            </div>
          );
        })}
...
export const DefinitionReveal: React.FC<PrimProps> = ({ props, theme: t }) => {
  ...
  return (
    <Wrap t={t} eyebrowText="definition">
      <div style={{ ...a, fontFamily: FONT.display, fontSize: 76, fontWeight: 700, color: t.accent }}>{props.term}</div>
      <div style={{ ...b, fontFamily: FONT.body, fontSize: 38, color: t.ink, lineHeight: 1.4, maxWidth: 1500 }}>{props.definition}</div>
    </Wrap>
  );
};
...
export const QuoteReveal: React.FC<PrimProps> = ({ props, theme: t }) => {
  ...
  return (
    <Wrap t={t}>
      <div style={{ ...a, fontFamily: FONT.display, fontSize: 58, color: t.ink, lineHeight: 1.3, maxWidth: 1500 }}>&ldquo;{props.quote}&rdquo;</div>
      <div style={{ ...b, fontFamily: FONT.mono, fontSize: 26, color: t.accent, letterSpacing: "0.1em" }}>— {props.attribution}</div>
    </Wrap>
  );
};
        ...
        {ev.map((e, i) => {
          ...
          return (
            <div key={i} style={{ ...st, display: "flex", gap: 22, alignItems: "flex-start" }}>
              <div style={{ fontFamily: FONT.mono, fontSize: 26, color: t.accent, minWidth: 190, letterSpacing: "0.08em" }}>{e.when}</div>
              <div style={{ width: 2, alignSelf: "stretch", background: t.line }} />
              <div>
                <div style={{ fontFamily: FONT.display, fontSize: 36, color: t.ink }}>{e.label}</div>
                {e.consequence && <div style={{ fontFamily: FONT.body, fontSize: 24, color: t.ink3, marginTop: 4 }}>{e.consequence}</div>}
              </div>
            </div>
          );
        })}
        ...
        {chain.map((c, i) => {
          ...
          return (
            <React.Fragment key={i}>
              <div style={{ ...st, border: `1px solid ${t.accent}55`, background: `${t.accent}14`, color: t.ink, borderRadius: 999, padding: "14px 24px", fontFamily: FONT.display, fontSize: 30 }}>{c}</div>
...
export const RecallFrame: React.FC<PrimProps> = ({ props, theme: t }) => {
  ...
  return (
    <Wrap t={t} eyebrowText="recall — reconstruct it before it appears">
      <div style={{ fontFamily: FONT.display, fontSize: 56, color: t.ink, maxWidth: 1500 }}>{props.prompt}</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 8 }}>
        {blanks.map((b, i) => (
          <div key={i} style={{ fontFamily: FONT.mono, fontSize: 32, color: revealed ? t.good : t.ink3, opacity: revealed ? o : 1 }}>
            {revealed ? b : b.replace(/[^:\s]/g, "_")}
          </div>
        ))}
      </div>
    </Wrap>
        ...
        {rows.map(([k, v], i) => {
          ...
          return (
            <div key={i} style={{ ...st, display: "flex", gap: 24 }}>
              ...
              <div style={{ fontFamily: FONT.body, fontSize: 32, color: t.ink, lineHeight: 1.4, maxWidth: 1300 }}>{v as string}</div>
            </div>
          );
        })}
      </div>
