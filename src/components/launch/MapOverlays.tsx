...
/* Two mutually-exclusive map overlays: this week's current affairs, or the
   PYQ heat-map. Both are real data; captions state exactly what's shown. */
export default function MapOverlays({ mode, loading, onMode, affairs, heat }: {
  ...
  const caption =
    ...
    : mode === "heat" ? `${heat.countries} countries across ${heat.total} PYQs · bigger, hotter = asked more. ${heat.top ? `Top: ${heat.top}.` : ""} Click for the questions.`
    : "";
